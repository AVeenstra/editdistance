...
\EndProcedure
%\algstore{MPIalgstore}
%\end{algorithmic}
%\end{multicols}
%\end{algorithm*}
%\begin{algorithm}
%\ContinuedFloat
%\caption{MPI control (cont.)} \label{MPIalg2}
%\begin{algorithmic}[1]
%\algrestore{MPIalgstore}
\Statex
\Function{execute\_kernel}{$max\_height$, $height$, $s_a$, $width$, $block$, $column$, $column\_next$} \label{MPIalg_functionbegin}
    ...
    \Else
        \State{\textproc{cl\_write\_memory}(to=$s_a.length + 1$, $column$)} \label{MPIalg_writecol3}
...
As mentioned earlier the if statement lines \ref{MPIalg_recvbegin} through \ref{MPIalg_recvend} manages the retrieval of columns from other nodes.
...
If the the process is at the first block of a pillar there is no need to copy the diagonals as the new block will be using \cref{begin}.
