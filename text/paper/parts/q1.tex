...
\Procedure{Block\_calc}{$id, width, s_a, s_b, height, d$}
    ...
    \For{$i$ is $0 \dots height$}
        ...
        \State{$\mathit{cell\_left} \gets d[x + 1] + 1$}
        \Statex
        \State{$d[x] \gets $\Call{minimum}{$cell\_up, \mathit{cell\_left}, cell\_d$}}
...
Each iterations is separated by a \Call{barrier}{\relax} operation.
